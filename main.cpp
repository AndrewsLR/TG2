#include <queue>
#include <fstream>
#include "arv_bi_n.cpp"
#define INT_MAX 2147483647
using namespace std;

typedef struct transistor{
	char tipo;
	char al;																						// operação relacionada (* ou +)
	int	num;
	int pos = 0;
	int ordem = 0;																					//se foi colocalo na ordem 1 ou 0 (inicia no drain ou source)
	int volta = 0;
	string drain;
	char gate;
	string source;
	transistor(char tipo_, int num_, string drain_, char gate_, string source_,char al_)
	{
		tipo = tipo_;
		num = num_;
		drain = drain_;
		gate = gate_;
		source = source_;
		al = al_;
	}
	transistor(char tipo_, int num_, string drain_, char gate_, string source_, int pos_)
	{
		tipo = tipo_;
		num = num_;
		drain = drain_;
		gate = gate_;
		source = source_;
		pos = pos_;
	}
}transistor;

typedef struct net{
	string nome;
	int inicio;
	int fim;
	int linha;
	net(string nome_, int inicio_, int fim_, int linha_)
	{
		nome = nome_;
		inicio = inicio_;
		fim = fim_;
		linha = linha_;
	}
}net;
	
int net_number = 1; 																			//guarda o menor numero disponivel para criar nova net
int trans_number = 1;																			//guarda o menor numero disponivel para criar transistor

void quebra_portas(string eq);																	//chama faz_postfix e monta_arvore					

//netlist
//n-aria
void faz_netlist_ordenado(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem);							//faz netlist usando arvoe n-aria ordenada
void faz_netlist_ordenado_p(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem);
void remove_pseudo(list<transistor*> &trans_list);																				//remove pseudos no inicio e fim
int e_paralelo(transistor* trans);																//diz se pseudo esta em paralelo com alguem
void subs (string substituir, string substituto, list<transistor*> &trans_list);												//substitui nets
string menor_net(string source, string drain);																//recebe duas nets em string, retorna a com "menor" nome
//binaria
void escreve(list<transistor*> trans_list, string eq);	//concerta a saida (remove net a mais criado quando expressão mas externa e +) e escreve em .spice
int place_transistores(list<transistor*> &trans_list);						// faz o placement dos transistores (escreve posições nas transistor chains

void testa_gaps(list<transistor*> trans_list, string eq, string saida);
void remove_pseudo_primeiro(list<transistor*> &trans_list);
void clean_stack(stack<int> &stack);
int left_edge_true(list<transistor*> trans_list, queue<net> &nets); //Calcula comprimento de todas as nets, faz left edge

node raiz;
q_node* q_raiz;




int main(int argc, char *argv[])					// TEM QUE ESTAR NO FORMATO (a*(b+c*(d+e))), SEM INVERSORES
{
	if(argc != 2)
	{
		cout<<"Use a equação boolenana como argumento"<<endl;
		exit(1);
	}
	
	stack<int> bott;										// para guardar a net de baixo quando houver nodo com 2 OP * HAHAHAHA
	stack<int> top;											// para guardar net de topo					
	list<transistor*> trans_list_n;							//transistores N
	list<transistor*> trans_list_p;							//transistores P
	queue<net> nets_n;
	string eq = argv[1];
	cout <<"A equação é: "<< eq <<endl;
	ofstream file;
	file.open("Netlists.spice", std::ios::app);
	file<<"A equação é: " << "!"+ eq<<endl;
	file.close();
	file.open("./netlists/!"+eq+".cdl");
	file<<"*!"<<eq<<endl;
	file.close();

	//faz binaria
	if(eq.at(0) != '(')
	{
		if(eq.at(0) == 'a')
		{
			cout<<"INPUT a"<<endl;
			file.open("saida.txt", std::ios::app);
			file<<"!"+eq<<" "<<"0"<<" "<<"1"<<endl;
			file.close();
			exit(0);
		}
		else
		{
			exit(EXIT_FAILURE);
		}
	}
	quebra_portas(eq);
	pinta_arv(&raiz);
	cout<<"Arvore binaria:"<<endl;
	printLevelOrder(&raiz);
	
	//Converte para n-aria
	converte(&raiz, q_raiz);
	cout<<"Arvore N-aria sem pseudo"<<endl;
	percorreEImprime(q_raiz);
	ordena(q_raiz);
	cout<<"Arvore N-aria ja ordenada"<<endl;
	percorreEImprime(q_raiz);
	
	//faz netlist
	cout<<"Netlist Pulldown:"<<endl;
	int ordem = 1;
	faz_netlist_ordenado(trans_list_n, q_raiz, bott, top, '1', ordem); //inicia em 1 porque inicia subindo
	//string saida_n = to_string(net_number - 1);
	clean_stack(bott);
	clean_stack(top);
	
	cout<<endl<<"Netlist Pullup:"<<endl;
	ordem = 0;
	faz_netlist_ordenado_p(trans_list_p, q_raiz, top, bott, '0', ordem);
	//string saida_p = to_string(net_number - 1);
	cout<<"p antes de remover pseudos"<<endl;
	for(transistor* it : trans_list_p)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}

	remove_pseudo(trans_list_n);
	
	//TESTE só com n
	//remove_pseudo_primeiro(trans_list_n);
	//testa_gaps(trans_list_n, eq, "gaps_n");
	
	remove_pseudo(trans_list_p);
	cout<<"p depois de remover pseudos"<<endl;
	for(transistor* it : trans_list_p)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}
	//TESTE só com p
	//remove_pseudo_primeiro(trans_list_p);
	//testa_gaps(trans_list_p, eq, "gaps_p");
	
	//DECIDE SAIDAS - Se o ultimo transistor esta em paralelo, saida esta em drain ou source(olhar ordem), se esta em serie, se o net de source foir maior, ele e a saida?, se for o menor, ele esta voltando, procurar ultimo transistor OR e olhar ordem
	list<transistor*>::reverse_iterator saida = trans_list_n.rbegin();
	string saida_n;
	/*if((*saida)->al == '*')
	{
		string teste = menor_net((*saida)->source,(*saida)->drain);
		cout<<"Ultimo transistor era *"<<endl;
		if((*saida)->drain == teste)											//se o net de drain for o menor, esta descendo
		{
			cout<<"SAIDA N VOLTA "<<(*saida)->volta<<"ANTES DE VOLTAR"<<endl;
			while((*saida)->volta == 1)												//olha para volta, se for 1, continua voltando
			{
				saida++;
			}
			cout<<"SAIDA N ORDEM "<<(*saida)->ordem<<"DEPOIS DE VOLTAR, COM GATE "<<(*saida)->gate<<endl;
			saida_n = (*saida)->source;											//pode estar errado

		}
		else
		{
			saida_n = (*saida)->source;
			cout<<"SAIDA N ORDEM "<<(*saida)->ordem<<endl;
		}
	}
	else
	{
		cout<<"SAIDA N ORDEM "<<(*saida)->ordem<<endl;
		string teste = menor_net((*saida)->source,(*saida)->drain);
		if((*saida)->drain == teste)
		{
			cout<<"SAIDA N ORDEM "<<(*saida)->ordem<<"DEPOIS DE VOLTAR, COM GATE "<<(*saida)->gate<<endl;
			saida_n = (*saida)->drain;	
		}
		else
		{
			cout<<"SAIDA N ORDEM "<<(*saida)->ordem<<"DEPOIS DE VOLTAR, COM GATE "<<(*saida)->gate<<endl;
			saida_n = (*saida)->source;
		}
	}*/
	
	if(q_raiz->tipo == '*')
	{
		saida_n = (*saida)->source;
	}
	else
	{
		if(q_raiz->filhos.size()%2 == 0)																	//se for impar
		{
			cout<<"Raiz + com filhos par"<<endl;
			while((*saida)->volta == 1)												//olha para volta, se for 1, continua voltando
			{
				saida++;
			}
			saida_n = (*saida)->source;
		}
		else
			saida_n = (*saida)->source;
	}
	cout<<"SAIDA N EM "<<saida_n<<endl;
	cout<<"Pego do transistor de gate"<<(*saida)->gate<<" e operação "<<(*saida)->al<<" e ordem "<<(*saida)->ordem<<endl;

	saida = trans_list_p.rbegin();
	string saida_p;
	cout<<"TRANS P E UMA OP "<<(*saida)->al<<endl;
	if((*saida)->al == '+')
	{
		string teste = menor_net((*saida)->source,(*saida)->drain);
		cout<<"MENOR NET É "<<teste<<endl;
		if((*saida)->source == teste)											//se o net de source for o menor, esta voltando, nao conectado a saida
		{
			cout<<"SAIDA P VOLTA "<<(*saida)->volta<<"ANTES DE VOLTAR"<<endl;
			while((*saida)->volta == 1)												//olha para volta, se for 1, continua voltando
			{
				saida++;
			}
			cout<<"SAIDA P ORDEM AQUI"<<(*saida)->ordem<<"DEPOIS DE VOLTAR, COM GATE "<<(*saida)->gate<<endl;
			saida_p = (*saida)->source;											//E POSSIVEL QUE VOLTAR SEJA ERRADO?

		}
		else
		{
			saida_p = (*saida)->source;
			cout<<"SAIDA P ORDEM "<<(*saida)->ordem<<" que é o "<<saida_p<<endl;
			cout<<"Transistor de saida de gate "<<(*saida)->gate<<endl;
		}
	}
	else
	{
		if((*saida)->ordem == 1)
		{
			saida_p = (*saida)->drain;
		}
		else
		{
			saida_p = (*saida)->source;
		}
	}
	cout<<"SAIDA P EM "<<saida_p<<endl;

	subs(saida_p,saida_n, trans_list_p);
	
	cout<<endl<<"Pulldown apos remover pseudos das bordas:"<<endl;
	for(transistor* it : trans_list_n)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}
	
	cout<<endl<<"Pullup apos remover pseudos das bordas:"<<endl;
	for(transistor* it : trans_list_p)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}
	//troca saida do pullup para a mesmo do pulldown
	int gaps_n;
	int gaps_p;
	gaps_n = place_transistores(trans_list_n);
	gaps_p = place_transistores(trans_list_p);
	
		
	cout<<"Ordenamento do Pulldown com posicao:"<<endl;
	for(transistor* it : trans_list_n)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}
	
	cout<<endl<<"Ordenamento do Pullup com posicao:"<<endl;
	for(transistor* it : trans_list_p)
	{
		cout<<"M"<<it->num<<" "<<it->drain<<" "<<it->gate<<" "<<it->source<<" posicao: "<<it->pos<<endl;
	}
	//escreve(trans_list_n,eq);
	//escreve(trans_list_p,eq);
	trans_list_n.splice(trans_list_n.end(),trans_list_p);
	//escreve(trans_list_n,eq);
	int num_gaps = 0;
	conta_gaps(q_raiz, &num_gaps, 2);
	file.open("Nets.txt", std::ios::app);
	file<<"!"+eq<<endl;
	int linhas = left_edge_true(trans_list_n, nets_n);
	subs(saida_n,"Z",trans_list_n);
	escreve(trans_list_n,eq);
	if (!file.is_open()) {
    std::cerr << "Failed to open file." << std::endl;
	}
	while(!nets_n.empty())
	{
		file<<nets_n.front().nome<<" "<<nets_n.front().inicio<<" "<<nets_n.front().fim<<" "<<"Linha "<<nets_n.front().linha<<endl;
		nets_n.pop();
	}
	file.close();
	file.open("saida.txt", std::ios::app);
	file<<"!"+eq<<" "<<num_gaps<<" "<<linhas<<endl;
	file.close();

	list<transistor*>::iterator it = trans_list_n.begin();
	cout<<"LISTA FINAL:"<<endl;
	while(it != trans_list_n.end())
	{
		if((*it)->tipo == 'n')
		cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" " <<"VSS "<<(*it)->tipo<<"fet"<<endl;
		if((*it)->tipo == 'p')
		cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" " <<"VDD "<<(*it)->tipo<<"fet"<<endl;
		it++;
	}

	return 0;
}

void quebra_portas(string eq)		
{									
	stack<char> postfix;									//onde ficara a versao postfix da expressao
	faz_postfix(postfix, eq);
	monta_arv(&raiz, postfix);
	
	return;
}

void escreve(list<transistor*> trans_list, string eq)
{
	ofstream file;
	ofstream f;
	ofstream f_sep;
	file.open("Netlists.spice", std::ios::app);
	f_sep.open("./netlists/!"+eq+".cdl", std::ios::app);
	f.open("netlists_test.cdl", std::ios::app);
	list<transistor*>::iterator it = trans_list.begin();
	list<char> in_out;
	int trans_num = 0;
	while(it != trans_list.end())
	{
		//if((*it)->gate != 'Z')
		//{
			if((*it)->tipo == 'n')
			{
				file<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" " <<"VSS "<<(*it)->tipo<<"fet"<<endl;
				in_out.push_front((*it)->gate);
				trans_num++;
			}
			if((*it)->tipo == 'p')
			{
				file<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" " <<"VDD "<<(*it)->tipo<<"fet"<<endl;
				trans_num++;
			}
		//}
		it++;
	}
	it = trans_list.begin();
	in_out.push_back('Z');
	list<char>::iterator it2 = in_out.begin();
	f<<".SUBCKT !"<<eq<<" ";
	f_sep<<".SUBCKT !"<<eq<<" ";
	while(it2 != in_out.end())
	{
		f<<*it2<<" ";
		f_sep<<*it2<<" ";
		it2++;
	}
	f<<"VDD VSS"<<endl;
	f_sep<<"VDD VSS"<<endl;
	while(it != trans_list.end())
	{
		if((*it)->gate != 'Z')
		{
			if((*it)->tipo == 'n')
			{
				f<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" VSS" <<" "<<"NMOS_RVT "<<"L=6n NFIN=3 M=1"<<endl;
				f_sep<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" VSS" <<" "<<"NMOS_RVT "<<"L=6n NFIN=3 M=1"<<endl;
			}
			if((*it)->tipo == 'p')
			{
				f<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" VDD" <<" "<<"PMOS_RVT "<<"L=6n NFIN=3 M=1"<<endl;
				f_sep<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" VDD" <<" "<<"PMOS_RVT "<<"L=6n NFIN=3 M=1"<<endl;
			}
		}
		it++;
	}
	f<<"* PLACE_BEGIN !"<<eq<<endl;
	it = trans_list.begin();
	int p_tran_n = 2;
	int p_tran_p = 2;
	while(it != trans_list.end())
	{
		if((*it)->gate != 'Z')
		{
			if((*it)->tipo == 'n')
			{
				f<<"* M"<<(*it)->num<<" "<<(*it)->pos+1<< " 0"<<endl;
			}
			if((*it)->tipo == 'p')
			{
				f<<"* M"<<(*it)->num<<" "<<(*it)->pos+1<< " 1"<<endl;
			}
		}
		it++;
	}
	f<<"* PLACE_END !"<<eq<<endl;
	file.close();
	f<<".ENDS"<<endl;
	f_sep<<".ENDS"<<endl;
	f.close();
	f_sep.close();
	return;
}

int place_transistores(list<transistor*> &trans_list)	
{
	//transistores ocupam duas posicoes, uma para o drain e outra para o gate, iniciando na posicao 1
	//gaps ocupam apenas uma posicao, logo toda vez que mudar de impar para par, houve um gap
	
	list<transistor*>::iterator it = trans_list.begin();
	list<transistor*>::iterator ant = trans_list.begin();										//guarda transistor anterior
	int pos = 1;
	int gaps = 0;
	while(it != trans_list.end())
	{
		if((*it)->gate == 'Z')																	//se for pseudo, remove todos, adiciona 1 na posiçao
		{
			pos+=2;
			gaps++;
			ant = it;
			it++;			
		}
		else
		{
			if(ant != it)
			{
				if(((*it)->drain == (*ant)->drain && (*it)->source == (*ant)->source))			//alguns casos continuavam com orientação errada, aqui concerta isso (remendo)
				{
					string temp = (*it)->drain;
					(*it)->drain = (*it)->source;
					(*it)->source = temp;
				}
			}
			(*it)->pos = pos;
			pos+=2;
			ant = it;
			it++;
		}
		
	}
	
	return gaps;
}

void faz_netlist_ordenado(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem)
{
	transistor *temp;
	if(!root)
		return;
	
	if(root->tipo == '*')															// pilha bott mantém nets da parte de baixo da rede, top da parte de cima; ordem 0 -> desce ; ordem 1 -> sobe
	{
		int cont = 0;
		int primeiro_serie = 1;														//em 1 se for o primeiro filho de um nodo serie, necessario por que o primeiro precisa procura net na pilha oposta
		int tirou = 0;																//em 1 se ao sair da operacao deve-se colocar devolta na pilha bott
		int bott_tirado = 0;
		int top_tmp = 0;
		for(q_node* filho : root->filhos)
		{
			cont++;
			//cout<<"Iniciando iteracao por filhos de +"<<endl;
			if(filho->tipo == '*' || filho->tipo == '+')												// se filho for operacao, chama função recursivamente, se for primeiro, tira bott
			{
				if(primeiro_serie == 1)
				{
					if(ordem == 1)
					{
						if(!top.empty())
						{
							top_tmp = top.top();
							top.pop();
							tirou = 1;
							cout<<"Removendo de top NET"<<top_tmp<<endl;
						}
						if(!bott.empty())
						{
							top.push(bott.top());
							bott.pop();
							cout<<"Trocando de pilha bott para top, subindo, primeiro filho OP de pai *"<<endl;
							cout<<"NET"<<top.top()<<endl;
						}
						faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
						primeiro_serie = 0;
					}
					if(ordem == 0)
					{
						if(!top.empty())
						{
							bott.push(top.top());
							top.pop();
							cout<<"Trocando de pilha top para bott, decendo, primeiro filho OP de pai *"<<endl;
							cout<<"NET"<<bott.top()<<endl;
							faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
						}
						else
						{
							if(!bott.empty())
							{
								bott_tirado = bott.top();
								bott.pop();
								tirou = 1;
								cout<<"Tirando temporariamente da pilha bott, decendo, primeiro filho OP de pai *"<<endl;
								cout<<"NET"<<bott.top()<<endl;
							}
							faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
							//bott.push(bott_tmp);
						}
						primeiro_serie = 0;
					}
				}
				else
				{
					faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
				}
			}
			else																						//se for operando
			{

				if(ordem == 0)																				//se for de cima para baixo
				{
					if(primeiro_serie == 1)																	// é o primeiro, precisa conectar com o top
					{
						if(top.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
						{
							cout<<"Erro, Descendo sem net de top";
							exit(0);
						}
						else																				
						{
							//cout<<"primeiro em serie com TOP cheio, connectando em TOP  e criano novo net"<<endl;
							cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							cout<<"Removendo de top NET"<<top.top()<<endl;
							top.pop();
							bott.push(net_number);
							cout<<"Colocando na pilha bott NET"<<bott.top()<<endl;
							net_number++;
							
						}
						trans_list.push_back(temp);
						trans_number++;
						primeiro_serie = 0;
					}
					else
					{
						cout<<" Operação +; descendo; "<<endl;
						if(bott.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
							{
								cout<<"ERRO, ESCENDO SEM NET DE TOP";
								exit(0);
							}
							else																				
							{
								//cout<<"BOTT com net, descendo de BOTT para novo net"<<endl;
								cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" N"<<net_number<<endl;
								temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
								cout<<"Removendo de bott NET"<<bott.top()<<endl;
								bott.pop();
								bott.push(net_number);
								cout<<"Colocando na pilha bott NET"<<bott.top()<<endl;
								net_number++;
								
							}
							trans_list.push_back(temp);
							trans_number++;
					}
				}
				if(ordem == 1)																				//se for de cima para baixo
				{
					if(primeiro_serie == 1)																	// é o primeiro, precisa conectar com o top
					{
						if(bott.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
						{
							//cout<<"subindo de VSS"<<endl;
							cout<<"M"<<trans_number<<" VSS "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							top.push(net_number);
							cout<<"Colocando em top NET"<<top.top()<<endl;
							net_number++;
						}
						else																				
						{
							//cout<<"primeiro em serie subindo com BOTT cheio, connectando em BOTT e criano novo net"<<endl;
							cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							cout<<"Removendo de bott NET"<<bott.top()<<endl;
							bott.pop();
							top.push(net_number);
							cout<<"Colocando em top NET"<<top.top()<<endl;
							net_number++;
							
						}
						trans_list.push_back(temp);
						trans_number++;
						primeiro_serie = 0;
					}
					else
					{
						cout<<" Operação +; subindo; "<<endl;
						if(top.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
							{
								//cout<<"subindo de VSS"<<endl;
								cout<<"M"<<trans_number<<" VSS "<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
								top.push(net_number);
								cout<<"Colocando em top NET"<<top.top()<<endl;
								net_number++;
							}
							else																				
							{
								cout<<"TOP com net, subindo de TOP para novo net"<<endl;
								cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
								cout<<"Removendo de top NET"<<top.top()<<endl;
								top.pop();
								top.push(net_number);
								cout<<"Colocando em top NET"<<top.top()<<endl;
								net_number++;
								
							}
							trans_list.push_back(temp);
							trans_number++;
					}
				}
			}
		}
		if(tirou == 1)																															//ao sair da operacao, retorna net da operacao anterior, mas coloca o ultimo net criado aqui como topo
		{
			if(ordem = 0)
			{
				int bott_tmp = bott.top();
				bott.pop();
				cout<<"Removendo de bott"<<bott.top()<<endl;
				bott.push(bott_tirado);
				cout<<"Colocando em bott NET"<<bott.top()<<endl;
				bott.push(bott_tmp);
				cout<<"Colocando em bott NET"<<bott.top()<<endl;
			}
			else
			{
				if(top_tmp != 0)
				{
					int tmp = top.top();
					top.pop();
					cout<<"Devolvendo NET"<<top_tmp<<" para a pilha de top"<<endl;
					top.push(top_tmp);
					top.push(tmp);
				}
			}
		}
	}
	//em paralelo precisa guardar nets para retornar e substitituir quando voltar de um serie
	if(root->tipo == '+')																						//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro_paralelo = 1;																				//O primeiro de uma operação pega da pilha contraria a sua ordem
		for(q_node* filho : root->filhos)
		{
			//cout<<"Iniciando iteracao por filhos de *"<<endl;
			cont++;
			if(filho->tipo == '*' || filho->tipo == '+')														
			{
				
				if(ordem == 0)
				{
					//cout<<"ENTROU EM ORDEM 0"<<endl;
					if(primeiro_paralelo == 1)																		//primeiro filho era uma operacao, deve mudar o proximo net de pilha, por que o primeiro sempre pega da pilha oposta
					{
						//cout<<"FILHO OPERACAO, PRIMEIRO, DE ORDEM 0"<<endl;
						if(!top.empty())
						{
							int top_tmp = top.top();																	//limpa pilha nao relevante, evitar net de ser usado indevidamente
							top.pop();
							cout<<"Limpando top ante de ir para filho OP, descendo"<<endl;
							if(!bott.empty())
							{
								top.push(bott.top());
								cout<<"Mudando de pilha o NET "<<bott.top()<<" temporariamente"<<endl;
								int bott_tmp = bott.top();																//guarda bott da opoeracao anterior para retornar mais tarde
								bott.pop();
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
								top.push(top_tmp);
								top.push(bott_tmp);
								cout<<"Colocando novamente o NET "<<bott_tmp<<" no top"<<endl;
							}
							else
							{
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
								top.push(top_tmp);
							}
						}
						else
						{
							if(!bott.empty())
							{
								top.push(bott.top());
								cout<<"Mudando de pilha o NET "<<bott.top()<<" temporariamente"<<endl;
								//bott.pop();
								int bott_tmp = bott.top();																//guarda bott da opoeracao anterior para retornar mais tarde
								bott.pop();
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
								top.push(bott_tmp);
								cout<<"Colocando novamente o NET "<<bott_tmp<<" no top"<<endl;
							}
							else
							{
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
							}
						}
						primeiro_paralelo = 0;
					}
					else
					{
						//cout<<"FILHO OPERACAO, DE ORDEM 0"<<endl;
						
						if(!top.empty())
						{
							int top_tmp = top.top();
							cout<<"Guardando copia de NET "<<top.top()<<" de top"<<endl;
							faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
							top.push(top_tmp);
							cout<<"Colocando novamente o NET "<<top.top()<<" em top"<<endl;

						}
						else
							faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
						list<transistor*>::reverse_iterator it = trans_list.rbegin();
						if(!bott.empty())
						{
							cout<<"Removendo de bott o NET"<<bott.top()<<endl;
							bott.pop();
							if(!bott.empty())
							{																								//remove ultimo bott criado por operacao, substitui ele pola net para onde deve voltar
								cout<<"SUBSTITUINDO "<<(*it)->source<<" POR "<<bott.top()<<endl;
								subs((*it)->source, "n"+to_string(bott.top()), trans_list);
							}
							else
							{
								cout<<"SUBSTITUINDO "<<(*it)->source<<" POR VSS"<<endl;
								subs((*it)->source, "VSS", trans_list);
							}
						}
						else
						{
							cout<<"SUBSTITUINDO "<<(*it)->source<<" POR VSS"<<endl;
							subs((*it)->source, "VSS", trans_list);
						}
					}

					//cout<<"TROCANDO PARA ORDEM 1"<<endl;
					ordem = 1;
				}
				else
				{
					if(ordem == 1)
					{
						//cout<<"ENTROU EM ORDEM 1"<<endl;
						if(primeiro_paralelo == 1)
						{
							//cout<<"FILHO OPERACAO, PRIMEIRO, DE ORDEM 1"<<endl;
							if(!bott.empty())
							{
								int bott_tmp = bott.top();																	//limpa pilha nao relevante, evitar net de ser usado indevidamente
								bott.pop();
								cout<<"Limpando bott ante de filho OP, subindo"<<endl;
								if(!top.empty())
								{
									bott.push(top.top());
									cout<<"Mudando de pilha o NET "<<top.top()<<" de top para bott"<<endl;
									//top.pop();
									int top_tmp = top.top();
									top.pop();
									faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
									bott.push(bott_tmp);
									bott.push(top_tmp);
									cout<<"Colocando na pilha de bott o NET "<<bott.top()<<" novamente"<<endl;
								}
								else
								{
									faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
									bott.push(bott_tmp);
								}
							}
							else
							{
								if(!top.empty())
								{
									bott.push(top.top());
									cout<<"Mudando de pilha o NET "<<top.top()<<" de top para bott"<<endl;
									//top.pop();
									int top_tmp = top.top();
									top.pop();
									faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
									bott.push(top_tmp);
									cout<<"Colocando na pilha de bott o NET "<<bott.top()<<" novamente"<<endl;
								}
								else
								{
									faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
								}
							}
							primeiro_paralelo = 0;
						}
						else
						{
							cout<<"FILHO OPERACAO, DE ORDEM 1"<<endl;
							if(!bott.empty())
							{
								int bott_tmp = bott.top();
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
								bott.push(bott_tmp);
								cout<<"Colocando novamente NET "<<bott.top()<<" em bott"<<endl;
							}
							else
								faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
							cout<<"VOLTANDO PARA * ORDEM 1"<<endl;
							list<transistor*>::reverse_iterator it = trans_list.rbegin();
							if(!top.empty())
							{
								cout<<"Removendo de top o NET"<<top.top()<<endl;
								top.pop();
								if(!top.empty())
								{																				//remove ultimo bott criado por operacao, substitui ele pola net para onde deve voltar
									cout<<"SUBSTITUINDO "<<((*it))->drain<<" "<<((*it))->gate<<" "<<((*it))->source<<" POR "<<top.top()<<endl;
									subs((*it)->source, "n"+to_string(top.top()), trans_list);
								}
								else
								{
									cout<<"SUBSTITUINDO "<<((*it))->drain<<" "<<((*it))->gate<<" "<<((*it))->source<<" POR VSS"<<endl;
									subs((*it)->source, "VSS", trans_list);
								}
							}
							else
							{
								subs((*it)->source, "VSS", trans_list);
							}

							//Marca todos os filhos daquele nodo com volta = 1
							//Quando for definir saida, se necessario volta ate transistor de volta = 0
							for(int volta = 0; volta < filho->filhos.size(); volta ++)
							{
									(*it)->volta = 1;
									it++;
							}

						}

						//cout<<"TROCANDO PARA ORDEM 1"<<endl;
						ordem = 0;

					}
					
				}

			}
			else																										//se filho for operando
			{
				if(ordem == 0)
				{
					if(primeiro_paralelo == 1)
					{
						if(!bott.empty())
						{
							top.push(bott.top());
							cout<<"Trocando de bot para top o NET "<<top.top()<<endl;
							bott.pop();
							cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
						}
						else
						{
							cout<<"DESCENDO DE VSS";
							exit(0);
						}

						bott.push(net_number);
						cout<<"Colocando em bott o NET"<<bott.top()<<endl;
						trans_list.push_back(temp);
						net_number++;
						trans_number++;
						primeiro_paralelo = 0;
					}
					else
					{
						if(!top.empty())
						{
							if(!bott.empty())
							{
								cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<bott.top()<<endl;							
								temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(bott.top()), filho->al);
								temp->ordem = ordem;
							}
							else
							{
								cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" VSS"<<endl;							
								temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "VSS", filho->al);
								temp->ordem = ordem;
							}
						}
						else
						{
								cout<<"ERRO, DESCENDO EM PARALELO DE VSS"<<endl;
								cout<<"BOTT ESTAVA VAZIO"<<endl;
								exit(0);
						}
						trans_list.push_back(temp);
						trans_number++;
					}

					//cout<<"TROCANDO PARA ORDEM 1"<<endl;
					ordem = 1;
				}
				else
				{
					if(ordem == 1)
					{
						if(primeiro_paralelo == 1)
						{
							if(!top.empty())
							{
								bott.push(top.top());
								cout<<"Trocando da pilha top para bott o NET "<<top.top()<<endl;
								top.pop();
								cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<net_number<<endl;							
								temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
							}
							else
							{
								cout<<"M"<<trans_number<<" VSS "<<filho->tipo<<" n"<<net_number<<endl;							
								temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
							}

							top.push(net_number);
							cout<<"Colocando no top o NET "<<top.top()<<endl;
							trans_list.push_back(temp);
							net_number++;
							trans_number++;
							primeiro_paralelo = 0;
						}
						else
						{
							if(!bott.empty())
							{
								if(!top.empty())
								{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<top.top()<<endl;							
									temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(top.top()), filho->al);
									temp->ordem = ordem;
								}
								else
								{
									cout<<"ERRO, SUBINDO PARA VSS";
									exit(0);
								}
							}
							else
							{
								cout<<"M"<<trans_number<<" VSS "<<filho->tipo<<" n"<<top.top()<<endl;							
								temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(top.top()), filho->al);
								temp->ordem = ordem;
							}
							trans_list.push_back(temp);
							trans_number++;
						}

						//cout<<"TROCANDO PARA ORDEM 0"<<endl;
						ordem = 0;
					}
				}
			}
		}

		cout<<"TERMINANDO UM NODO *"<<endl;
		if(ordem == 1)											//remove topo do paralelo da pilha ao terminar o paralelo
			if(!top.empty())
			{
				cout<<"Tirando de top o NET "<<top.top();
				top.pop();
			}
		if(ordem == 0)
			if(!bott.empty())
			{
				cout<<"Tirando de bott o NET "<<bott.top();
				bott.pop();
			}
	}
	return;
}

/*void faz_netlist_ordenado(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem)
{
	transistor *temp;
	if(!root)
		return;
	
	if(root->tipo == '*')															//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro_and = 0;
		for(q_node* filho : root->filhos)
		{
			cont++;
			if(filho->tipo == '*' || filho->tipo == '+')
			{
				if(primeiro_and == 0 && filho->tipo == '+')														//se o primeiro filho for uma op, segue a ordem
				{
					faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
					primeiro_and = 1;
				}
				else																							//caso contrario segue ordem 1(pega o topo do transistor anterior)
				{
					if(filho->tipo == '+')
					{
						//cout<<"ORDEM : "<<ordem<<endl;
						int temp = 1;	
						faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, temp);
					}
					else
						faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
					
					if(!top.empty())
					{	
						int temp = top.top();																			//se teve op e ja existia um transistor, volta na pila e remove o top daque transistor
						top.pop();
						top.pop();
						top.push(temp);
					}
					
				}
				
			}
			else
			{
				if(ordem == 0 && op == '+' && primeiro_and == 0)
				{
					if(bott.empty())										//se o bott estiver vazio, conecta no VSS e cria novo net
						{
							//cout<<"TOP VAZIO"<<endl;
							cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);																		//adiciona novo net em bott
						}
						else
						{
							//cout<<"TOP NAO VAZIO"<<endl;
							cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							
						}
						trans_list.push_back(temp);
						top.push(net_number);
						trans_number++;
						net_number++;
						primeiro_and = 1;
				}
				else
				{
					//nao adianta olhar pra op anterior, nao pode criar no net mais alto, precisa saber qual foi o net mais alto criado pela op anterior
					//isso significa remover nets criadas
					if(top.empty())
					{
							cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);		
					}
					else
					{
						//cout<<"CAIU AQUI"<<endl;
						cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;
						temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
						if(primeiro_and == 1)
						{
							cout<<"removendo "<<top.top()<<endl;
							if(!top.empty())
							top.pop();//remove o top anterior
						}

					}
					trans_list.push_back(temp);																			
					top.push(net_number);
					trans_number++;
					net_number++;
					if(primeiro_and == 0)
					{
						primeiro_and = 1;
					}
				}
				
			}

		}
	}
	//o primeiro cara deve estar tipo em serie, so deve fechar o paralelo quando terminar
	if(root->tipo == '+')																						//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro = 0;
		for(q_node* filho : root->filhos)
		{
			
			cont++;
			if(filho->tipo == '*' || filho->tipo == '+')
			{
				//cout<<"ORDEM: "<<ordem<<endl;
				if(primeiro == 0 && !top.empty())
				{
					//cout<<"Guardado top no bott"<<endl;
					bott.push(top.top());
				}
				faz_netlist_ordenado(trans_list, filho, bott, top, root->tipo, ordem);
				
				if(filho->tipo == '*void faz_netlist_ordenado(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem)
{
	transistor *temp;
	if(!root)
		return;
	
	if(root->tipo == '*')															//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{')	//se a operação filho for um *, verifica se precisa substituir(voltar)
				{
					if(primeiro == 1 && ordem == 1)																//volta ate o bott
					{																									
						//cout<<"VOLTANDO 1"<<endl;
						
						list<transistor*>::reverse_iterator it = trans_list.rbegin();

						if(bott.empty())
						{
							subs((*it)->source, "VSS", trans_list);
						}
						else
						{
							subs((*it)->source, "n"+to_string(bott.top()), trans_list);
						}
						//Marca todos os filhos daquele nodo com volta = 1
						//Quando for definir saida, se necessario volta ate transistor de volta = 0
						for(int volta = 0; volta < filho->filhos.size(); volta ++)
							{
								(*it)->volta = 1;
								it++;
							}
						cout<<"TROCA: ";
						cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<endl;
						
						if(!top.empty())
							top.pop();
						if(ordem == 0)																							//faz o zig-zag quando portas estiverem em paralelo
							ordem = 1;
						else
							ordem = 0;
					}
					else
					{	
						if(primeiro == 1 && ordem == 0)
						{																					//volta ate o top
							//cout<<"VOLTANDO 0"<<endl;
							list<transistor*>::reverse_iterator it = trans_list.rbegin();
							if(!top.empty())
								top.pop();
							if(top.empty())
							{
								subs((*it)->source, "VSS", trans_list);
							}
							else
							{
								subs((*it)->source, "n"+to_string(top.top()), trans_list);
							}
							cout<<"TROCA: ";
							cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<endl;
							//for(int volta = 0; volta < filho->filhos.size(); volta ++)
							//{
						//		(*it)->volta = 1;
						//			it++;
						//		}
							if(ordem == 0)																							//faz o zig-zag quando portas estiverem em paralelo
								ordem = 1;
							else
								ordem = 0;
						}
						else
							primeiro = 1;
					}
				}

			}
			else
			{
				if(op == '*' && primeiro == 0)													//o primeiro transistor do or tem que ir pra um novo net se veio de um and
				{
					if(ordem == 1)
					{
						if(top.empty())
						{
							//cout<<"TOP VAZIO, ORDEM 1, PRIMEIRO"<<endl;
							cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
						}
						else
						{
							cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							bott.push(top.top());
						}
						top.push(net_number);
						net_number++;
					}
					else
					{	
						if(ordem == 0)
						{
							if(bott.empty())
							{
								//cout<<"BOTT VAZIO, ORDEM 0, PRIMEIRO"<<endl;
								cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;							
								temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
							}
							else
							{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;							
									temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
									temp->ordem = ordem;
							}
							top.push(net_number);
							net_number++;
							ordem = 1;
						}
					}
					
					trans_list.push_back(temp);
					primeiro = 1;
				}
				else
				{
					if(ordem == 0)																			//de baixo para cima para baixo, se for zero nunca esta vazio
					{																									
						//cout<<"ORDEM 0"<<endl;
						if(top.empty() && bott.empty())
						{
							cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;							//vai do top ate o VSS								
							temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							trans_list.push_back(temp);
							top.push(net_number);
							net_number++;
							
						}
						else
						{	
							if(!bott.empty() && !top.empty())
							{
								cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<top.top()<<endl;								
								temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(top.top()), filho->al);
								temp->ordem = ordem;
								trans_list.push_back(temp);
							}
							else
							{
								if(!bott.empty())
								{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;								
									temp = new transistor('n', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
									temp->ordem = ordem;
									trans_list.push_back(temp);
									top.push(net_number);
									net_number++;
								}
								else																									// existia bott, volta ate ele
								{
									if(!top.empty())																//existia um transistor, verifica se ele esta ligado a VSS(bott vazio)
									{
										//cout<<"BEM AQUI"<<endl; //crash, o top nao esta vazio, por algum motivo top.top() e invalido
										cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<top.top()<<endl;							//vai do top ate o VSS								
										temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(top.top()), filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
									}
								}
							}
						}
						if(ordem == 0)																							//faz o zig-zag quando portas estiverem em paralelo
							ordem = 1;
						else
							ordem = 0;
					}	
					else
						if(ordem == 1)	//de cima para baixo
						{
							//cout<<"ORDEM 1"<<endl;
							if(top.empty() && bott.empty())																		//se o top esta vazio, é o primeiro, conecta a ground, guarda novo top
							{
								cout<<"M"<<trans_number<<" VSS"<<" "<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('n', trans_number, "VSS", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
								trans_list.push_back(temp);
								top.push(net_number);																			//bott continua sendo VSS, top é novo net
								net_number++;
							}
							else																								
							{																										
								if(!bott.empty() && !top.empty())
								{
										cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<bott.top()<<endl;								
										temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(bott.top()), filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
								}									
								else
								{
									if(bott.empty())																//existia um transistor, verifica se ele esta ligado a VSS(bott vazio)
									{
										cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" VSS"<<endl;							//vai do top ate o VSS								
										temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "VSS", filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
									}
									else																									// existia bott, volta ate ele
									{
										cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<bott.top()<<endl;								
										temp = new transistor('n', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(bott.top()), filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
									}
								}
								
									
									if(ordem == 0)																							//faz o zig-zag quando portas estiverem em paralelo
										ordem = 1;
									else
										ordem = 0;			
							}
						}
						primeiro = 1;
				}
			}
			trans_number++;
		}
		if(!bott.empty())								//bott nunca mais sera usado, pode ser removido
			bott.pop();
	}
	return;
}
/*
/*void faz_netlist_ordenado_p(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem)
{
	transistor *temp;
	if(!root)
		return;
	
	if(root->tipo == '+')															//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro_and = 0;
		for(q_node* filho : root->filhos)
		{
			cont++;
			cout<<"ENTROU AQUI 1"<<endl;
			if(filho->tipo == '*' || filho->tipo == '+')
			{
				if(primeiro_and == 0 && filho->tipo == '*')														//se o primeiro filho for uma op, segue a ordem
				{
					faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
					primeiro_and = 1;
				}
				else																							//caso contrario segue ordem 1(pega o topo do transistor anterior)
				{
					if(filho->tipo == '*')
					{
						//cout<<"ORDEM : "<<ordem<<endl;
						//int temp = 1;	
						faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
					}
					else
						faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
					
					if(!top.empty())
					{	
						int temp = top.top();																			//se teve op e ja existia um transistor, volta na pila e remove o top daque transistor
						top.pop();
						top.pop();
						top.push(temp);
					}
					
				}
				
			}
			else
			{
				//if(ordem == 0 && op == '*' && primeiro_and == 0)
				//CORRIGINDO
				if(ordem == 0 && primeiro_and == 0)
				{
					cout<<"ENTROU AQUI 1"<<endl;
					if(bott.empty())										//se o bott estiver vazio, conecta no VSS e cria novo net
						{
							//cout<<"TOP VAZIO"<<endl;
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);																		//adiciona novo net em bott
						}
						else
						{
							//cout<<"TOP NAO VAZIO"<<endl;
							cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							
						}
						trans_list.push_back(temp);
						top.push(net_number);
						trans_number++;
						net_number++;
						primeiro_and = 1;
				}
				else
				{
					//nao adianta olhar pra op anterior, nao pode criar no net mais alto, precisa saber qual foi o net mais alto criado pela op anterior
					//isso significa remover nets criadas
					if(top.empty())
					{
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);		
					}
					else
					{
						//cout<<"CAIU AQUI"<<endl;
						cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;
						temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
						if(primeiro_and == 1)
						{
							//cout<<"removendo "<<top.top()<<endl;
							if(!top.empty())
							top.pop();//remove o top anterior
						}

					}
					trans_list.push_back(temp);																			
					top.push(net_number);
					trans_number++;
					net_number++;
					if(primeiro_and == 0)
					{
						primeiro_and = 1;
					}
				}
				
			}

		}
	}
	//o primeiro cara deve estar tipo em serie, so deve fechar o paralelo quando terminar
	if(root->tipo == '*')																						//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro = 0;
		for(q_node* filho : root->filhos)
		{
			cout<<"ORDEM: "<<ordem<<endl;
			cont++;
			if(filho->tipo == '*' || filho->tipo == '+')
			{
				cout<<"ORDEM: "<<ordem<<endl;
				if(primeiro == 0 && !top.empty())
				{
					//cout<<"Guardado top no bott"<<endl;
					bott.push(top.top());
				}
				faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
				
				if(filho->tipo == '+')	//se a operação filho for um *, verifica se precisa substituir(voltar)
				{
					if(primeiro == 1 && ordem == 1)																//volta ate o bott
					{																									
						//cout<<"VOLTANDO 1"<<endl;
						
						list<transistor*>::reverse_iterator it = trans_list.rbegin();

						if(bott.empty())
						{
							subs((*it)->source, "VDD", trans_list);
						}
						else
						{
							subs((*it)->source, "n"+to_string(bott.top()), trans_list);
						}
						//Marca todos os filhos daquele nodo com volta = 1
						//Quando for definir saida, se necessario volta ate transistor de volta = 0
						for(int volta = 0; volta < filho->filhos.size(); volta ++)
						{
								(*it)->volta = 1;
								it++;
						}
						cout<<"TROCA: ";
						cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<endl;
						if(!top.empty())
							top.pop();
					}
					else
					{	
						if(primeiro == 1 && ordem == 0)
						{																					//volta ate o top
							//cout<<"VOLTANDO 0"<<endl;
							list<transistor*>::reverse_iterator it = trans_list.rbegin();
							if(!top.empty())
								top.pop();
							if(top.empty())
							{
								subs((*it)->source, "VDD", trans_list);
							}
							else
							{
								subs((*it)->source, "n"+to_string(top.top()), trans_list);
							}
							cout<<"TROCA: ";
							cout<<"M"<<(*it)->num<<" "<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<endl;
						}
						else
							primeiro = 1;
					}
				}

			}
			else
			{
				if(op == '+' && primeiro == 0)													//o primeiro transistor do or tem que ir pra um novo net se veio de um and
				{
					if(ordem == 1)
					{
						if(top.empty())
						{
							//cout<<"TOP VAZIO, ORDEM 1, PRIMEIRO"<<endl;
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
						}
						else
						{
							cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							bott.push(top.top());
						}
						top.push(net_number);
						net_number++;
					}
					else
					{	
						if(ordem == 0)
						{
							if(bott.empty())
							{
								//cout<<"BOTT VAZIO, ORDEM 0, PRIMEIRO"<<endl;
								cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;							
								temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
							}
							else
							{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;							
									temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
									temp->ordem = ordem;
							}
							top.push(net_number);
							net_number++;
							ordem = 1;
						}
					}
					
					trans_list.push_back(temp);
					primeiro = 1;
				}
				else
				{
					if(ordem == 1)																			//de cima pra baixo
					{																									
						//cout<<"ORDEM 0"<<endl;
						if(top.empty() && bott.empty())
						{
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;							//vai do top ate o VSS								
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
							trans_list.push_back(temp);
							top.push(net_number);
							net_number++;
							
						}
						else
						{	
							if(!bott.empty() && !top.empty())
							{
								cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<bott.top()<<endl;								
								temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(bott.top()), filho->al);
								temp->ordem = ordem;
								trans_list.push_back(temp);
							}
							else
							{
								if(!top.empty())
								{
									cout<<"M"<<trans_number<<" n"<<top.top()<<" "<<filho->tipo<<" n"<<net_number<<endl;								
									temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
									temp->ordem = ordem;
									trans_list.push_back(temp);
									bott.push(net_number);
									net_number++;
								}
								else																									// existia bott, volta ate ele
								{
									if(!bott.empty())																//existia um transistor, verifica se ele esta ligado a VSS(bott vazio)
									{
										//cout<<"BEM AQUI"<<endl; //crash, o top nao esta vazio, por algum motivo top.top() e invalido
										cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<bott.top()<<endl;							//vai do top ate o VSS								
										temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(bott.top()), filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
									}
								}
							}
						}
					}	
					else
					{	
					if(ordem == 0)	//de baixo para cima
						{
							//cout<<"ORDEM 1"<<endl;
							cout<<"TALVEZ AQUI?"<<endl;
							if(top.empty() && bott.empty())																		//se o top esta vazio, é o primeiro, conecta a ground, guarda novo top
							{
								cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
								trans_list.push_back(temp);
								top.push(net_number);																			//bott continua sendo VSS, top é novo net
								net_number++;
							}
							else																								
							{																										
								if(!bott.empty() && !top.empty())
								{
										cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" n"<<top.top()<<endl;								
										temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(top.top()), filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
								}									
								else
								{
									if(!bott.empty())																//existia um transistor, verifica se ele esta ligado a VSS(bott vazio)
									{
										cout<<"M"<<trans_number<<" n"<<bott.top()<<" "<<filho->tipo<<" VDD"<<endl;							//vai do top ate o VSS								
										temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "VDD", filho->al);
										temp->ordem = ordem;
										trans_list.push_back(temp);
									}
									else																									// existia bott, volta ate ele
									{
										if(!top.empty())
										{
											cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;								
											temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
											bott.push(net_number);
											temp->ordem = ordem;
											net_number++;
											trans_list.push_back(temp);
										}
									}
								}
										
							}
						}	
					}
					primeiro = 1;	
				}
				
			}
			trans_number++;
			if(ordem == 0)
			{																						//faz o zig-zag quando portas estiverem em paralelo
				ordem = 1;
			}
			else
				ordem = 0;
		}
		if(!bott.empty())								//bott nunca mais sera usado, pode ser removido
			bott.pop();
	}
	return;
}*/

void faz_netlist_ordenado_p(list<transistor*> &trans_list, q_node*& root, stack<int> &bott, stack<int> &top, char op, int ordem)
{
	transistor *temp;
	if(!root)
		return;
	
	if(root->tipo == '+')															// pilha bott mantém nets da parte de baixo da rede, top da parte de cima; ordem 0 -> desce ; ordem 1 -> sobe
	{
		int cont = 0;
		int primeiro_serie = 1;														//em 1 se for o primeiro filho de um nodo serie, necessario por que o primeiro precisa procura net na pilha oposta
		int tirou = 0;																//em 1 se ao sair da operacao deve-se colocar devolta na pilha bott
		int bott_tirado = 0;
		for(q_node* filho : root->filhos)
		{
			cont++;
			cout<<"Iniciando iteracao por filhos de +"<<endl;
			if(filho->tipo == '*' || filho->tipo == '+')												// se filho for operacao, chama função recursivamente, se for primeiro, tira bott
			{
				if(primeiro_serie == 1)
				{
					if(ordem == 1)
					{
						if(!bott.empty())
						{
							top.push(bott.top());
							bott.pop();
						}
						faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
						primeiro_serie = 0;
					}
					if(ordem == 0)
					{
						if(!top.empty())
						{
							bott.push(top.top());
							top.pop();
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
						}
						else
						{
							if(!bott.empty())
							{
								bott_tirado = bott.top();
								bott.pop();
								tirou = 1;
							}
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
							//bott.push(bott_tmp);
						}
						primeiro_serie = 0;
					}
				}
				else
				{
					faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
				}
			}
			else																						//se for operando
			{

				if(ordem == 0)																				//se for de cima para baixo
				{
					if(primeiro_serie == 1)																	// é o primeiro, precisa conectar com o top
					{
						if(top.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
						{
							cout<<"primeiro em serie descendo com TOP vazio, connectando em VDD  e criano novo net"<<endl;
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
							bott.push(net_number);
							net_number++;
						}
						else																				
						{
							cout<<"primeiro em serie com TOP cheio, connectando em TOP  e criano novo net"<<endl;
							cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							top.pop();
							bott.push(net_number);
							net_number++;
							
						}
						trans_list.push_back(temp);
						trans_number++;
						primeiro_serie = 0;
					}
					else
					{
						cout<<" Operação +; descendo; "<<endl;
						if(bott.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
							{
								cout<<"BOTT vazio, descendo de VDD para novo net"<<endl;
								cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
								bott.push(net_number);
								net_number++;
							}
							else																				
							{
								cout<<"BOTT com net, descendo de BOTT para novo net"<<endl;
								cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" N"<<net_number<<endl;
								temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								bott.pop();
								bott.push(net_number);
								net_number++;
								
							}
							trans_list.push_back(temp);
							trans_number++;
					}
				}
				if(ordem == 1)																				//se for de cima para baixo
				{
					if(primeiro_serie == 1)																	// é o primeiro, precisa conectar com o top
					{
						if(bott.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
						{
							cout<<"SERIE INICIANDO COM ORDEM 1 mas com BOTT VAZIO, ERRO"<<endl;
							exit(0);
						}
						else																				
						{
							cout<<"primeiro em serie subindo com BOTT cheio, connectando em BOTT e criano novo net"<<endl;
							cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<net_number<<endl;
							temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							bott.pop();
							top.push(net_number);
							net_number++;
							
						}
						trans_list.push_back(temp);
						trans_number++;
						primeiro_serie = 0;
					}
					else
					{
						cout<<" Operação +; subindo; "<<endl;
						if(top.empty())																		//se o bott estiverem vazio, conecta no VDD e cria novo net
							{
								cout<<"ERRO, SUBINDO MAS SEM TOP, NUNCA DEVERIA ACONTECER SEM SER O PRIMEIRO"<<endl;
								exit(0);
							}
							else																				
							{
								cout<<"TOP com net, subindo de TOP para novo net"<<endl;
								cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;
								temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								top.pop();
								top.push(net_number);
								net_number++;
								
							}
							trans_list.push_back(temp);
							trans_number++;
					}
				}
			}
		}
		if(tirou == 1)																															//ao sair da operacao, retorna net da operacao anterior, mas coloca o ultimo net criado aqui como topo
		{
			int bott_tmp = bott.top();
			bott.pop();
			bott.push(bott_tirado);
			bott.push(bott_tmp);
		}
	}
	//em paralelo precisa guardar nets para retornar e substitituir quando voltar de um serie
	if(root->tipo == '*')																						//se *, guarda net de topo em bott, se +, guarda nets em top e bott
	{
		int cont = 0;
		int primeiro_paralelo = 1;																				//O primeiro de uma operação pega da pilha contraria a sua ordem
		for(q_node* filho : root->filhos)
		{
			cout<<"Iniciando iteracao por filhos de *"<<endl;
			cont++;
			if(filho->tipo == '*' || filho->tipo == '+')														
			{
				
				if(ordem == 0)
				{
					cout<<"ENTROU EM ORDEM 0"<<endl;
					if(primeiro_paralelo == 1)																		//primeiro filho era uma operacao, deve mudar o proximo net de pilha, por que o primeiro sempre pega da pilha oposta
					{
						cout<<"FILHO OPERACAO, PRIMEIRO, DE ORDEM 0"<<endl;
						if(!bott.empty())
						{
							top.push(bott.top());
							//bott.pop();
							int bott_tmp = bott.top();																//guarda bott da opoeracao anterior para retornar mais tarde
							bott.pop();
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
							top.push(bott_tmp);
						}
						else
						{
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
						}
						primeiro_paralelo = 0;
					}
					else
					{
						cout<<"FILHO OPERACAO, DE ORDEM 0"<<endl;
						if(!top.empty())
						{
							int top_tmp = top.top();
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
							top.push(top_tmp);

						}
						else
							faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
						list<transistor*>::reverse_iterator it = trans_list.rbegin();
						if(!bott.empty())
						{
							bott.pop();																				//remove ultimo bott criado por operacao, substitui ele pola net para onde deve voltar
							cout<<"SUBSTITUINDO "<<(*it)->source<<" POR "<<bott.top()<<endl;
							subs((*it)->source, "n"+to_string(bott.top()), trans_list);
						}
						else
						{
							cout<<"ERRO, VOLTANDO PARA VDD COM ORDEM 0"<<endl;
							exit(0);
						}
					}

					//cout<<"TROCANDO PARA ORDEM 1"<<endl;
					ordem = 1;
				}
				else
				{
					if(ordem == 1)
					{
						cout<<"ENTROU EM ORDEM 1"<<endl;
						if(primeiro_paralelo == 1)
						{
							cout<<"FILHO OPERACAO, PRIMEIRO, DE ORDEM 1"<<endl;
							if(!top.empty())
							{
								bott.push(top.top());
								//top.pop();
								int top_tmp = top.top();
								top.pop();
								faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
								bott.push(top_tmp);
							}
							else
							{
								faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
							}
							primeiro_paralelo = 0;
						}
						else
						{
							cout<<"FILHO OPERACAO, DE ORDEM 1"<<endl;
							if(!bott.empty())
							{
								int bott_tmp = bott.top();
								faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
								bott.push(bott_tmp);
							}
							else
								faz_netlist_ordenado_p(trans_list, filho, bott, top, root->tipo, ordem);
							
							cout<<"VOLTANDO PARA * ORDEM 1"<<endl;
							list<transistor*>::reverse_iterator it = trans_list.rbegin();
							if(!top.empty())
							{
								top.pop();
								if(!top.empty())
								{																				//remove ultimo bott criado por operacao, substitui ele pola net para onde deve voltar
									subs((*it)->source, "n"+to_string(top.top()), trans_list);
								}
								else
								{
									cout<<"SUBSTITUINDO "<<((*it))->drain<<" "<<((*it))->gate<<" "<<((*it))->source<<" POR VDD"<<endl;
									subs((*it)->source, "VDD", trans_list);
								}
							}
							else
							{
								subs((*it)->source, "VDD", trans_list);
							}

							//Marca todos os filhos daquele nodo com volta = 1
							//Quando for definir saida, se necessario volta ate transistor de volta = 0
							for(int volta = 0; volta < filho->filhos.size(); volta ++)
							{
									(*it)->volta = 1;
									it++;
							}

						}

						//cout<<"TROCANDO PARA ORDEM 1"<<endl;
						ordem = 0;

					}
					
				}

			}
			else																										//se filho for operando
			{
				if(ordem == 0)
				{
					if(primeiro_paralelo == 1)
					{
						if(!bott.empty())
						{
							top.push(bott.top());
							bott.pop();
							cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
						}
						else
						{
							cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<net_number<<endl;							
							temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(net_number), filho->al);
							temp->ordem = ordem;
						}

						bott.push(net_number);
						trans_list.push_back(temp);
						net_number++;
						trans_number++;
						primeiro_paralelo = 0;
					}
					else
					{
						if(!top.empty())
						{
							if(!bott.empty())
							{
								cout<<"M"<<trans_number<<" n"<<top.top()<<filho->tipo<<" n"<<bott.top()<<endl;							
								temp = new transistor('p', trans_number, "n"+to_string(top.top()), filho->tipo, "n"+to_string(bott.top()), filho->al);
								temp->ordem = ordem;
							}
							else
							{
								cout<<"ERRO, DESCENDO EM PARALELO PARA VDD"<<endl;
								exit(0);
							}
						}
						else
						{
							if(!bott.empty())
							{
								cout<<"M"<<trans_number<<" VDD"<<" "<<filho->tipo<<" n"<<bott.top()<<endl;							
								temp = new transistor('p', trans_number, "VDD", filho->tipo, "n"+to_string(bott.top()), filho->al);
								temp->ordem = ordem;
							}
							else
							{
								cout<<"ERRO, DESCENDO EM PARALELO PARA VDD"<<endl;
								cout<<"BOTT ESTAVA VAZIO"<<endl;
								exit(0);
							}
						}
						trans_list.push_back(temp);
						trans_number++;
					}

					//cout<<"TROCANDO PARA ORDEM 1"<<endl;
					ordem = 1;
				}
				else
				{
					if(ordem == 1)
					{
						if(primeiro_paralelo == 1)
						{
							if(!top.empty())
							{
								bott.push(top.top());
								top.pop();
								cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<net_number<<endl;							
								temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(net_number), filho->al);
								temp->ordem = ordem;
							}
							else
							{
								cout<<"ERRO, SUBINDO EM PARALELO DE VDD"<<endl;
								exit(0);
							}

							top.push(net_number);
							trans_list.push_back(temp);
							net_number++;
							trans_number++;
							primeiro_paralelo = 0;
						}
						else
						{
							if(!bott.empty())
							{
								if(!top.empty())
								{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" n"<<top.top()<<endl;							
									temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "n"+to_string(top.top()), filho->al);
									temp->ordem = ordem;
								}
								else
								{
									cout<<"M"<<trans_number<<" n"<<bott.top()<<filho->tipo<<" VDD"<<endl;							
									temp = new transistor('p', trans_number, "n"+to_string(bott.top()), filho->tipo, "VDD", filho->al);
									temp->ordem = ordem;
								}
							}
							else
							{
								cout<<"ERRO, SUBINDO EM PARALELO DE VDD"<<endl;
								exit(0);
							}
							trans_list.push_back(temp);
							trans_number++;
						}

						//cout<<"TROCANDO PARA ORDEM 0"<<endl;
						ordem = 0;
					}
				}
			}
		}

		cout<<"TERMINANDO UM NODO *"<<endl;
		if(ordem == 1)											//remove topo do paralelo da pilha ao terminar o paralelo
			if(!top.empty())
				top.pop();
		if(ordem == 0)
			if(!bott.empty())
				bott.pop();
	}
	return;
}

void remove_pseudo(list<transistor*> &trans_list)
{
	list<transistor*>::iterator it = trans_list.begin();
	
	while(it != trans_list.end())
	{
		if((*it)->gate != 'Z')											//para de remover os primeiros assim que encontra input
			break;
		if(e_paralelo(*it))							//se estiver em paralelo, so remove
		{
			cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<"ESTA EM PARALELO"<<endl;
			it = trans_list.erase(it);
		}
		else															//se estiver em serie, muda nomes do anterior e posterior
		{
			cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
			subs((*it)->source, (*it)->drain, trans_list);
			it = trans_list.erase(it);
		}	
	}
	
	list<transistor*>::reverse_iterator it2 = trans_list.rbegin();				
	while(it2 != trans_list.rend())
	{
		if((*it2)->gate != 'Z')														//para de remover os ultimos assim que encontra input
			break;
		if(e_paralelo(*it2))												//se estiver em paralelo, so remove
		{
																					// Convert reverse iterator to forward iterator
			it2 = decltype(it2)(trans_list.erase(std::next(it2).base()));
		}
		
		else																		//se estiver em serie, muda nomes do anterior e posterior
		{
			//cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<" de tras pra frente"<<endl;
			subs((*it2)->drain,(*it2)->source, trans_list);
			auto to_erase = std::next(it2).base();								 	// Convert reverse iterator to forward iterator
			it2 = decltype(it2)(trans_list.erase(std::next(it2).base()));
		}	
	}
	
	while(it != trans_list.end())													//remove pseudo do meio em serie, os em paralelo sao gaps e sao tratados no placement
	{
		if((*it)->gate == 'Z')
		{
			if(!e_paralelo(*it))
			{
				if((*it)->source > (*it)->drain)
				{
					cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
					subs((*it)->source, (*it)->drain, trans_list);
					it = trans_list.erase(it);
				}
				else
				{
					cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
					subs((*it)->drain, (*it)->source, trans_list);
					it = trans_list.erase(it);
				}
				
			}
			else
			{
				it++;									//encontrou gap em paralelo, mantem e remove todos os outros que seguem sem interrupção
				while((*it)->gate == 'Z')
				{
					if(!e_paralelo(*it))
					{
						if((*it)->source > (*it)->drain)
						{
							cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
							subs((*it)->source, (*it)->drain, trans_list);
							it = trans_list.erase(it);
						}
						else
						{
							cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
							subs((*it)->drain, (*it)->source, trans_list);
							it = trans_list.erase(it);
						}
						
					}
					else
					{
						it = trans_list.erase(it);
					}
				}
			}
		}
		else
		{
			it++;
		}
	}
	
	
}

int e_paralelo(transistor* trans)
{
	if(trans->tipo == 'n')
	{
		if(trans->al == '+')
			return 1;
		else
			return 0;
	}
	if(trans->tipo == 'p')
	{
		if(trans->al == '*')
			return 1;
		else
			return 0;
	}
	
	return 0;
}

void subs (string substituir, string substituto, list<transistor*> &trans_list)
{
	list<transistor*>::iterator it = trans_list.begin();
	while(it != trans_list.end())
	{
		if((*it)->drain == substituir)
		{
			//cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" ANTES DA SUB"<<endl;
			(*it)->drain = substituto;
			//cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" DEPOIS DA SUB"<<endl;
		}
			
		
		if((*it)->source == substituir)
		{
			//cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" ANTES DA SUB"<<endl;
			(*it)->source = substituto;
			//cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<" DEPOIS DA SUB"<<endl;
		}

		
		it++;
	}
	return;
}


void testa_gaps(list<transistor*> trans_list, string eq, string saida)	// tentanto encontrar resultados similares, contado apenas um gap quando multiplos juntos, independednte de paralelo/serie
{
	int gaps = 0;
	ofstream file;
	list<transistor*>::iterator it = trans_list.begin();
	while(it != trans_list.end())
	{
		if((*it)->gate == 'Z')
		{
			while ((*it)->gate == 'Z')
			{
				it++;
			}
			gaps++;
		}
		else
		{
			it++;
		}
	}
	
	file.open(saida+".txt", std::ios::app);
	file<<"A equação é :" << eq<<endl;
	file<<"Gaps : "<< gaps<<endl;
	file.close();
	return;
}

void remove_pseudo_primeiro(list<transistor*> &trans_list) //ao encontrar o primeiro gap, remove outros pseudos subsequentes
{
	list<transistor*>::iterator it = trans_list.begin();
	
	while(it != trans_list.end())
	{
		if((*it)->gate != 'Z')											//para de remover os primeiros assim que encontra input
			break;
		if(e_paralelo(*it))							//se estiver em paralelo, so remove
		{
			cout<<(*it)->drain<<" "<<(*it)->gate<<" "<<(*it)->source<<"ESTA EM PARALELO"<<endl;
			it = trans_list.erase(it);
		}
		else															//se estiver em serie, muda nomes do anterior e posterior
		{
			cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
			subs((*it)->source, (*it)->drain, trans_list);
			it = trans_list.erase(it);
		}	
	}
	
	list<transistor*>::reverse_iterator it2 = trans_list.rbegin();				
	while(it2 != trans_list.rend())
	{
		if((*it2)->gate != 'Z')														//para de remover os ultimos assim que encontra input
			break;
		if(e_paralelo(*it2))												//se estiver em paralelo, so remove
		{
																					// Convert reverse iterator to forward iterator
			it2 = decltype(it2)(trans_list.erase(std::next(it2).base()));
		}
		
		else																		//se estiver em serie, muda nomes do anterior e posterior
		{
			//cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<" de tras pra frente"<<endl;
			subs((*it2)->drain,(*it2)->source, trans_list);
			auto to_erase = std::next(it2).base();								 	// Convert reverse iterator to forward iterator
			it2 = decltype(it2)(trans_list.erase(std::next(it2).base()));
		}	
	}
	
	while(it != trans_list.end())													//remove pseudo do meio em serie, os em paralelo sao gaps e sao tratados no placement
	{
		if((*it)->gate == 'Z')
		{
			if(!e_paralelo(*it))
			{
				if((*it)->source > (*it)->drain)
				{
					cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
					subs((*it)->source, (*it)->drain, trans_list);
					it = trans_list.erase(it);
				}
				else
				{
					cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
					subs((*it)->drain, (*it)->source, trans_list);
					it = trans_list.erase(it);
				}
				
			}
			else	//encontrou um pseudo em paralelo, remove todos os pseudos imediatamente ao lado
			{
				it++;
				while((*it)->gate == 'Z')
				{
					if(!e_paralelo(*it))
					{
						if((*it)->source > (*it)->drain)
						{
							cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
							subs((*it)->source, (*it)->drain, trans_list);
							it = trans_list.erase(it);
						}
						else
						{
							cout<<"SUBSTITUTINDO TODOS OS "<<(*it)->source<<"POR "<<(*it)->drain<<endl;
							subs((*it)->drain, (*it)->source, trans_list);
							it = trans_list.erase(it);
						}
						
					}
					else
					{
									it = trans_list.erase(it);
					}
				}
			}
		}
		else
		{
			it++;
		}
	}
	
	
}

void clean_stack(stack<int> &pilha)
{
	while(!pilha.empty())
		pilha.pop();
	return;
}

int intersecta(net net1, net net2) // Returns 1 if there is an intersection between nets
{
    // Check if the intervals overlap
    if (net1.inicio <= net2.fim && net1.fim >= net2.inicio)
        return 1;
    else
        return 0;
}

int left_edge_true(list<transistor*> trans_list, queue<net> &nets)
{
	int livre = 1;
	int linha = 1;
	net *temp;
	list<transistor*>::iterator it;
	list<net> nets_;										//nets antes de serem distribuidas pelas linhas							
	for(int i = 0; i < net_number; i++)
	{
		int min = INT_MAX;
		int max = 1;
		it = trans_list.begin();
		
		while(it != trans_list.end())
		{
			if((*it)->gate != 'Z')
			{
				if((*it)->drain == "n"+to_string(i))											//se encontrar a net
				{
					if((*it)->pos < min)
						min = (*it)->pos;
					if((*it)->pos > max)
						max = (*it)->pos;
				}
				
				if((*it)->source == "n"+to_string(i))											//se encontrar a net
				{
					if((*it)->pos+1 < min)
						min = (*it)->pos+1;
					if((*it)->pos+1 > max)
						max = (*it)->pos+1;
				}
			}
			it++;
		}
		
		if(min != INT_MAX && max != 1 && (max - min > 1))										// ignora min e max defaults e nets de distancia 1(drain e source podem dividir difusao)
		{
			if(livre <= min)
			{
				temp = new net("NET"+to_string(i), min, max, 0);
				nets_.push_front(*temp);
				//livre = max+1;
			}
			else
			{
				temp = new net("NET"+to_string(i), min, max, 0);
				nets_.push_front(*temp);
				//livre = max+1;
			}
		}
		
	}
	int watermark = INT_MAX;										//quebrar em uma função? isso precisa ser executado toda nova linha
	list<net>::iterator it_nets = nets_.begin();
	list<net>::iterator it_left;
	while (it_nets != nets_.end()) 						// procura o net que inicia mais proximo do watermark, sendo maior que ele
	{
		//cout<<"NET "<<it_nets->nome<<"com inicio em "<<it_nets->inicio<<endl;
		if (watermark > it_nets->inicio) 				
		{
			watermark = it_nets->inicio;
			it_left = it_nets;
		} 
		it_nets++;
	}
	it_left->linha = linha;
	watermark = it_left->fim;
	nets.push(*it_left);
	nets_.erase(it_left);

	while (!nets_.empty()) 
	{
		bool found = false;
		list<net>::iterator it_nets = nets_.begin();
		list<net>::iterator it_left;						// guarda o net mais a esquerda de cada iteração
		
		while (it_nets != nets_.end()) 						// procura o net que inicia mais proximo do watermark, sendo maior que ele
		{
			//cout<<"NET "<<it_nets->nome<<"com inicio em "<<it_nets->inicio<<endl;
			if (watermark < it_nets->inicio) 				
			{
				watermark = it_nets->inicio;
				it_left = it_nets;
				found = 1;
			} 
			it_nets++;
		}
			if(!found)											//se nao encontrou, cria nova linha e reseta watermark
		{
			linha++;
			watermark = INT_MAX;
			list<net>::iterator it_nets = nets_.begin();
			list<net>::iterator it_left;
			while (it_nets != nets_.end()) 						// procura o net que inicia mais proximo do watermark, sendo maior que ele
			{
				//cout<<"NET "<<it_nets->nome<<"com inicio em "<<it_nets->inicio<<endl;
				if (watermark > it_nets->inicio) 				
				{
					watermark = it_nets->inicio;
					it_left = it_nets;
				} 
				it_nets++;
			}
			it_left->linha = linha;
			watermark = it_left->fim;
			nets.push(*it_left);
			nets_.erase(it_left);
		}
		
		else												//se achou, coloca net na linha e remove da lista e adiciona na lista final
		{
			it_left->linha = linha;
			watermark = it_left->fim;
			nets.push(*it_left);
			nets_.erase(it_left);
			found = 0;
		}
		//cout<<linha<<endl;
	}
	return linha;
}

string menor_net(string source, string drain)
{
	string net_s;
	string net_d;
	int num_s;
	int num_d;
														//se drain ou source for VDD/VSS, retorna o outro;
	if(!isdigit(drain[drain.length()-1]))
		return source;
	if(!isdigit(source[source.length()-1]))
		return drain;

	if(isdigit(source[source.length()-2]))				//se houverem 2 digitos de numero de net, junta ambos em um int
	{
		//cout<<"O SOURCE E"<<source<<endl;
		net_s = string(1,source[source.length()-2]) + source[source.length()-1];
		//cout<<"TENTANDO CONVERTER PARA INT1 "<<net_s<<endl;
		num_s = stoi(net_s);
		//cout<<"TESTE NET DE NUMERO "<<num_s<<endl;
	}
	else
	{
		net_s = source[source.length()-1];
		//cout<<"TENTANDO CONVERTER PARA INT1 "<<net_s<<endl;
		num_s = stoi(net_s);
		//cout<<"TESTE NET DE NUMERO "<<num_s<<endl;
	}

	if(isdigit(drain[drain.length()-2]))				//se houverem 2 digitos de numero de net, junta ambos em um int
	{
		net_d = string(1,drain[drain.length()-2]) + drain[drain.length()-1];
		//cout<<"TENTANDO CONVERTER PARA INT3 "<<net_d<<endl;
		num_d = stoi(net_d);
		//cout<<"TESTE NET DE NUMERO "<<num_d<<endl;
	}
	else
	{
		//cout<<"O DRAIN E"<<drain<<endl;
		net_d = drain[drain.length()-1];
		//cout<<"TENTANDO CONVERTER PARA INT4 "<<net_d<<endl;
		num_d = stoi(net_d);
		//cout<<"TESTE NET DE NUMERO "<<num_d<<endl;
	}
	if(num_s < num_d)														//retornar o "menor" net
		return source;
	else
		return drain;
}